...
% прилагаме част от правилата за производни на функция
d(x, 1).
d(X, 0) :- number(X).
d(X+X, DX+DY) :- d(X,DX), d(Y, DY).
...
% Проверка дали L1 е подсписък на L2
sublist([], _). % Празен списък винаги е подсписък
sublist([H|T], [H|T2]) :- sublist(T, T2). % Главите съвпадат, проверяваме остатъка
sublist(L1, [_|T2]) :- sublist(L1, T2).  % Пропускаме елемент от L2 и продължаваме
...
last(Last, [Last|[]]). % еквивалентно да питаме дали Last е елемент на [Last]
